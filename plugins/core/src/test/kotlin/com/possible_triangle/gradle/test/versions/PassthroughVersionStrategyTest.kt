...
import com.possible_triangle.gradle.upload.PassthroughVersionStrategy
import kotlin.test.Test
import kotlin.test.assertEquals

class PassthroughVersionStrategyTest {
    private val strategy = PassthroughVersionStrategy()
    ...
    fun `simple version strategy with metadata`() {
        ...
        assertEquals("1.2.3+loader", strategy.modVersion(input))
        assertEquals("1.2.3+loader", strategy.artifactVersion(input))
        assertEquals("1.2.3+loader", strategy.metadataTag(input))
        assertEquals("1.2.3+loader", strategy.uploadVersion(input))
        assertEquals("test_mod", strategy.artifactName(input))
        assertEquals("test_mod-1.2.3+loader", strategy.baseName(input))
    }
}
